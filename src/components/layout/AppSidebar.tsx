  ...
  return <Sidebar collapsible="icon">
      <SidebarHeader className="p-4">
        <div className="flex items-center gap-3 group-data-[collapsible=icon]:justify-center">
          ...
          <span className="text-lg font-bold text-gradient-emerald group-data-[collapsible=icon]:hidden">Kwiqi</span>
        </div>
      </SidebarHeader>
          ...
          <SidebarGroupContent>
            <SidebarMenu>
              {navItems.map(item => <SidebarMenuItem key={item.path}>
                  <SidebarMenuButton isActive={location.pathname === item.path} onClick={() => navigate(item.path)} tooltip={item.title} className="transition-all duration-200">
